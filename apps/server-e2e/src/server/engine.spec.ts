...
describe('Engine (e2e)', () => {
  ...
  it('English error in try create project with wrong admin secret key', async () => {
    try {
      await user.getEngineApi().engineProjectsControllerCreateOne(
        {
          public: false,
      ...
      const errData = getErrorData<EngineError>(err);
      expect(errData).not.toEqual(null);
      expect(errData?.code).toEqual(EngineErrorEnum.Engine013);
  ...
  it('English validation errors in sign-up with not equals password', async () => {
    try {
      await user.getEngineApi().engineControllerSignUp(
        {
  ...
  it('English errors in sign-in not exists user', async () => {
    try {
      await user.getEngineApi().engineControllerSignIn(
        {
      ...
      const errData = getErrorData<EngineError>(err);
      expect(errData).not.toEqual(null);
      expect(errData?.code).toEqual(EngineErrorEnum.Engine001);
  ...
  it('English errors in sign-in not verified email', async () => {
    try {
      ...
      const errData = getErrorData<EngineError>(err);
      expect(errData).not.toEqual(null);
      expect(errData?.code).toEqual(EngineErrorEnum.Engine012);
  ...
  it('English errors in change password about empty password and confirmPassword fields', async () => {
    try {
      await user.getEngineApi().engineControllerUpdateProfile(
        {
          password: '',
          confirmPassword: '',
        },
  ...
  it('Change password', async () => {
    const { data: changePasswordResult } = await user.getEngineApi().engineControllerUpdateProfile(
      {
        password: user.randomUser.newPassword,
        confirmPassword: user.randomUser.newPassword,
  ...
  it('English errors in sign-in with old password', async () => {
    try {
      ...
      const errData = getErrorData<EngineError>(err);
      expect(errData).not.toEqual(null);
      expect(errData?.code).toEqual(EngineErrorEnum.Engine002);
  ...
  it('English errors in refresh tokens method called with wrong refresh-token', async () => {
    try {
      await user.getEngineApi().engineControllerRefreshTokens(
        {
          fingerprint: user.randomUser.id,
      ...
      const errData = getErrorData<EngineError>(err);
      expect(errData).not.toEqual(null);
      expect(errData?.code).toEqual(EngineErrorEnum.Engine007);
      expect(errData?.message).toEqual('Refresh token not provided');
    }
  });

  it('Should refresh tokens successfully', async () => {
    const { data: refreshTokensResult } = await user.getEngineApi().engineControllerRefreshTokens(
      {
        fingerprint: user.randomUser.id,
  ...
  it('English errors in sign-out method called with wrong refresh-token', async () => {
    try {
      await user.getEngineApi().engineControllerSignOut(
        { refreshToken: randomUUID() },
        {
          headers: {
            'x-client-id': project.randomUser.id,
  ...
  it('Sign-out', async () => {
    const { data: signOutResult } = await user.getEngineApi().engineControllerSignOut(
      { refreshToken: userTokens.refreshToken },
      {
        headers: {
          'x-client-id': project.randomUser.id,
