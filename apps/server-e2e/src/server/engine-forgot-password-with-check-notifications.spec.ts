...
describe('Engine forgot password with check notifications (e2e)', () => {
  ...
  it('Create project', async () => {
    const { data: createOneResult } = await admin.getEngineApi().engineProjectsControllerCreateOne({
      public: false,
  ...
  it('As admin get verify code from notifications and use it for verify as user', async () => {
    ...
    const { data: completeSignUpResult } = await user
      .getEngineApi()
      .engineControllerCompleteSignUp({ code, fingerprint: user.randomUser.id });
  ...
  it('Send link for restore forget password with change password logic', async () => {
    ...
    const { data: forgotPasswordResult } = await user.getEngineApi().engineControllerForgotPassword(
      { email: user.randomUser.email },
      {
        headers: {
          'x-client-id': project.randomUser.id,
  ...
  it('As admin get complete forgot password code from notifications and use it for change password', async () => {
    ...
    const { data: completeForgotPasswordResult } = await user.getEngineApi().engineControllerCompleteForgotPassword({
      code,
