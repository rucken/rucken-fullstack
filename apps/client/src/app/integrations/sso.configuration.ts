...
export class SsoIntegrationConfiguration implements SsoConfiguration {
  ...
  oAuthProviders(): Observable<OAuthProvider[]> {
    return this.ruckenRestSdkAngularService.getSsoApi().ssoOAuthControllerOauthProviders();
  }
  ...
  updateProfile(data: SsoUpdateProfileInput): Observable<void | null> {
    ...
    return (data.picture ? this.filesService.getPresignedUrlAndUploadFile(data.picture) : of('')).pipe(
      ...
      mergeMap((picture) => {
        return this.ruckenRestSdkAngularService.getSsoApi().ssoControllerUpdateProfile({
      ...
      mergeMap(() => this.ruckenRestSdkAngularService.getSsoApi().ssoControllerProfile()),
      mergeMap((newData) => {
        if (
          oldData?.picture &&
