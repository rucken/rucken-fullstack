...
if (!parsedEnvs.RUCKEN_MINIO_ACCESS_KEY) {
  throw new Error('RUCKEN_MINIO_ACCESS_KEY not set');
}

if (!parsedEnvs.RUCKEN_MINIO_SECRET_KEY) {
  throw new Error('RUCKEN_MINIO_SECRET_KEY not set');
}
...
parsedEnvs.RUCKEN_KEYV_URL = postgresUrl.replace('?schema=public', '');

parsedEnvs.RUCKEN_CLIENT_MINIO_URL = `https://${supabaseName}.supabase.co/storage/v1/s3`;
parsedEnvs.RUCKEN_MINIO_URL = `https://${supabaseName}.supabase.co/storage/v1/s3`;
parsedEnvs.RUCKEN_MINIO_SERVER_HOST = `${supabaseName}.supabase.co`;
parsedEnvs.RUCKEN_SUPABASE_URL = `https://${supabaseName}.supabase.co`;

parsedEnvs.RUCKEN_SUPABASE_KEY = supabaseAnonKey;
parsedEnvs.RUCKEN_DISABLE_SERVE_STATIC = 'true';
